...
# There is a significant association but what's driving significant association?
# ...Pairwise chi-squared testing

# Check variances (> |2| significant deviation from H0) 
sal_counts_chi$stdres

# Create tables for pairwise chi-square tests
chi_table_pool_cascade <- sal_count_table %>% 
  filter(channel_class %in% c("Pool", "Cascade")) %>% 
  select(-channel_class)
  
chi_table_pool_sidechan <- sal_count_table %>% 
  filter(channel_class %in% c("Pool", "Side Channel")) %>% 
  select(-channel_class)
 
 chi_table_cascade_sidechan <- sal_count_table %>% 
  filter(channel_class %in% c("Cascade", "Side Channel")) %>% 
  select(-channel_class)
 
 # Perform the three pairwise chi-square tests for comparison
 pool_cascade_chi <- chisq.test(chi_table_pool_cascade)
 pool_cascade_chi
 
 pool_sidechan_chi <- chisq.test(chi_table_pool_sidechan)
 pool_sidechan_chi
 
 cascade_sidechan_chi <- chisq.test(chi_table_cascade_sidechan)
 cascade_sidechan_chi
```
