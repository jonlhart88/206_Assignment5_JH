...
# Create Histogram to test for normality
 ggplot(data = sal_weights_class, aes(x = weight)) +
  ...
  scale_y_continuous(expand = c(0,0),
                     limits = c(0, 130)) +
   theme_bw() +
   labs(title = "Salamanader Mean Weight Normality Testing (1)",
...
ggplot(data = sal_weights_class, aes(sample = weight)) +
  ...
  facet_wrap(~channel_class) +
  theme_bw() +
  labs(title = "Salamanader Mean Weight Normality Testing (2)",
       subtitle = "Mack Creek, OR (2017)",
...
ggplot() +
   ...
   geom_errorbar(data = sal_summary,
                 ...
                 color = "black") +
  theme_bw() +
  labs(title = "Mean Salamander Size by Channel Class (2017)",
...
```
Mean salamander weight (g) differed significantly between the three creek channel classifications (pool, cascade, side-channel) by a one-way ANOVA test F(`r aov_outputs[1]`, `r aov_outputs[2]`) = `r round(aov_outputs[7],2)`, *p* = `r round(aov_outputs[9],3)`). However, Post-hoc Tukey's HSD revealed that only the side-channel and pool channel classifications differed significantly 
(*p* =`r sal_post_hoc$channel_class[12]`). Though mean salamander weight differed significantly between the pool and side-channel sections of Mack Creek, the effect size is small (Cohen's *d* = `r round(creek_section_d$estimate, 2)`). 
